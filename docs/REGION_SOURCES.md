...
## ДОПОВНЕННЯ 2026-09-07: Чернігів і Хмельницький — перевірка «швидких перемог»

**ЧЕРНІГІВ: попередній вердикт «blocked» був ПОМИЛКОВИЙ — його винесено по чужому домену.**

- `chernigivoblenergo.com.ua` (через «g») — **захоплений домен букмекера**, title:
  «Регистрация, полные условия ставок, обзор линии БК Betwinner Украина». Саме на нього
  натрапило дослідження 2026-08-28.
- `chernihivoblenergo.com.ua` (через «h») — **справжній оператор**, title «Чернігівобленерго»,
  HTTP 200, `robots.txt` = `User-agent: * / Allow: /` (закрито лише Yandex). Формально дозволено.
- АЛЕ: після кількох запитів сервер починає **скидати TLS-зʼєднання**
  (`Connection reset by peer` на 94.131.247.142:443). Це обмеження частоти, а не бан за політикою.
  Адаптер тут можливий, але має ходити рідко й з великими паузами; пробувати далі варто
  з іншого дня і не серіями.

**ФОРМАТ ГРАФІКІВ — головна перешкода для обох областей.**
Перевірено архіви офіційних Telegram-каналів за весь минулий сезон:

| Канал | Період перевірки | Текстових таблиць | Фото |
|---|---|---|---|
| `@chernigivoblenergo` | лют. 2025 – січ. 2026 | **0** | 140 |
| `@khmelnytskoblenergo` | лист. 2025 – кві. 2026 | 1 (лише поправка) | 118 |

Тобто **обидва публікують ГПВ картинками**. Наш текстовий парсер (`lib/telegram.mjs`) їм не
допомагає. Той єдиний «текстовий» пост Хмельницького — не таблиця, а уточнення:
«обсяг відключень на 26 лютого додатково збільшено: підчерга 2.1 — відключення розпочнеться
раніше о 13:00».

**Хмельницький має відкритий архів файлів:** `hoe.com.ua` (robots.txt = 404, тобто заборон немає),
сторінка `/page/arhiv-grafikiv-pogodinnih-vidkljuchen-2026` віддає **114 файлів** —
переважно PNG (`/Content/Uploads/YYYY/MM/fileYYYYMMDDHHMMSSmmm.png`), є щонайменше один PDF.

**ВИСНОВОК: усі області, що лишились, — картинкові.** Наступний крок, який їх відмикає
(Чернігів, Хмельницький, Закарпаття, імовірно Львів) — це читач сітки з зображення.
Спосіб перевірити полярність (що означає зафарбована клітинка) існує: у Хмельницького текстові
поправки називають конкретну підчергу й годину, тож розібране зображення того ж дня можна
звірити з текстом і встановити семантику емпірично, а не вгадувати.
